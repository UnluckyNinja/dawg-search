...
let { findWords } = prepareSearch(words.value)

const result = shallowRef<ReturnType<typeof findWords>>()

watch([searchTextInput, words], ([newText, newWords], [_, oldWords])=>{
  if (oldWords !== newWords) {
    ;({ findWords } = prepareSearch(newWords))
  }

  result.value = refineMatches(findWords(newText))
}, {immediate: true})
